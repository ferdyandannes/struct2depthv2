...
g = open("Motion/objmotion1_ori.txt", "r")
#print(f.read())
...
for i in range(len(testsite_array)):
	...
	print("pre_x = ", float(pre_x), " pre_y = ", float(pre_y))
	print("pre_x_obj = ", float(pre_x_obj), " pre_y_obj = ", float(pre_y_obj))
	...
	cv2.line(traj_new, (int(pre_x_obj), int(pre_y_obj)),(int(pre_x_obj-obj_traj_past_x), int(pre_y_obj-obj_traj_past_y)), (255,0,255), int(scale*0.3))
	#cv2.line(traj_new, (int(pre_x_obj), int(pre_y_obj)),(int(obj_traj_past_x-pre_x), int(obj_traj_past_y-pre_y)), (255,0,255), int(scale*0.3))
	cv2.imshow("object", traj_new)

	obj_traj_past_x = pre_x_obj
	obj_traj_past_y = pre_y_obj
...
#cv2.imwrite("0117_ori.jpg", traj_new)
#cv2.imwrite("0117_tra.jpg", traj_new2)
cv2.waitKey(0)